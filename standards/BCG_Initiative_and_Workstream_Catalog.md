...
**What Changed (v3.4 — I-78 Registration):** Added I-78 (K8s Cluster Storage Architecture & Persistent Volume Strategy) per Gregory approval 2026-05-03. Decision-then-implementation initiative covering storage class strategy (Longhorn vs Ceph/Rook vs local-path vs hybrid), replication factor, role of expansion 1 TB NVMe tier vs SN7100 boot tier, and backup target for the 4-node `nuc-cluster` K8s worker fleet. Hard gate: no format of the four expansion NVMes (1 TB each, installed via Bejavr M.2-to-PCIe adapters per ADR-002) until I-78 architecture decision is recorded. Hardware foundation documented in ADR-002 (`standards/ADR-002_nuc-cluster_Hardware_Selection.md`, APPROVED 2026-05-03). Supports I-65 (On-Prem AI Suite) for any distributed-PV requirements. **Counts:** **66 active initiatives** (was 65; +1 = I-78). 12 removed total (unchanged). Next I-slot: **I-79** (was I-78). No changes to subprojects or workstreams beyond WS-10 coverage list.
...
**66 active initiatives** (I-01 through I-78, minus 12 removed: I-06, I-07, I-20, I-25, I-26, I-30, I-31, I-32, I-47, I-56, I-61, I-64).
...
| **I-78** | **K8s Cluster Storage Architecture & Persistent Volume Strategy** | **Jason (decision) / Greg (arch co-authority)** | **Planned** | **P4** | **Q2 '26 (decision) → Q3 '26 (implementation)** | **NEW 2026-05-03.** Decision-then-implementation initiative for the 4-node `nuc-cluster` K8s worker fleet's persistent storage architecture. **Hardware foundation: ADR-002** (`standards/ADR-002_nuc-cluster_Hardware_Selection.md`, APPROVED 2026-05-03) documents the 4x NUC9VXQNX nodes with 64 GB ECC RAM, 2 TB SN7100 boot tier, 1 TB expansion NVMe per node via Bejavr M.2-to-PCIe adapters, and Intel X710-T2L 10 GbE NICs. **Phase 1 (decision, Q2 '26):** Choose storage class strategy from {local-path, Longhorn, Ceph/Rook, hybrid}; choose replication factor (2x or 3x); decide role separation between SN7100 boot tier and 1 TB expansion tier; decide backup target (file server, off-cluster S3-compatible, none). **Phase 2 (implementation, Q3 '26):** Format expansion drives per chosen architecture, deploy storage operator, create initial StorageClasses, document operational runbook. **Hard gate:** No format of any of the four expansion 1 TB NVMes until Phase 1 decision is recorded — drives are physically installed but unformatted, awaiting decision. Cluster aggregate storage post-implementation: 8 TB SN7100 boot + 4 TB expansion NVMe over 10 GbE fabric. Supports I-65 (On-Prem AI Suite) for any distributed-PV requirements. Tier 3 internal workloads only — no Tier 1–2 client data permitted on this cluster. Plan: TBD — author at Active flip per `Initiative_Plan_Discipline.md`. |
...
| WS-10 | Efficiency & AI Deployment | Greg/Jason | Active | P0/P4 | I-09, I-22, I-27, I-28, I-43, I-44, I-45, I-46, I-48, I-49, I-51, I-52, I-60, I-65, I-66, I-67, I-71, I-72, I-73, I-75, I-76, I-77, **I-78** | v3.4: Added I-78 to coverage list. |
...
| P4 | **9** | I-27, I-28, I-43, I-48, I-51, I-60, I-65, I-72, **I-78** |
| P4-001 | 0 | |
...
**Total:** 66 active initiative primary-project assignments.
...
- **Initiatives:** I-01 through I-## (sequential, never reuse removed numbers). Next: **I-79**
...
|---------|------|-------------|
| **3.4** | **2026-05-03** | **I-78 Registration.** Added I-78 (K8s Cluster Storage Architecture & Persistent Volume Strategy) per Gregory approval 2026-05-03. Decision-then-implementation initiative covering storage class strategy (Longhorn vs Ceph/Rook vs local-path vs hybrid), replication factor, role of expansion 1 TB NVMe tier vs SN7100 boot tier, and backup target for the 4-node `nuc-cluster` K8s worker fleet. Hard gate: no format of the four expansion NVMes (1 TB each, installed via Bejavr M.2-to-PCIe adapters per ADR-002) until Phase 1 decision is recorded. Hardware foundation documented in ADR-002 (`standards/ADR-002_nuc-cluster_Hardware_Selection.md`, APPROVED 2026-05-03). Supports I-65 (On-Prem AI Suite) for any distributed-PV requirements. Added I-78 to WS-10 coverage list. P4 active initiative count 8 → 9. **Counts:** **66 active initiatives** (was 65; +1 = I-78). 12 removed total (unchanged). Next I-slot: **I-79** (was I-78). No changes to subprojects. |
