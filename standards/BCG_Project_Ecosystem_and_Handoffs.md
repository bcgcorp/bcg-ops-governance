...
Before writing any `[FROM:]` label — whether by a human or an automated process — answer these three questions in order. All three must be answered correctly before the label is written.

**Question 1 — Origin check (determines FROM)**
In which Claude project was this work actually produced?
...
**Question 2 — Destination check (determines routing, not FROM)**
Which project needs to receive and act on this output?
This determines where the handoff is *pasted*, not what goes in the FROM field.
FROM and routing destination are independent. A P0-produced directive pasted into P4 is still `FROM: P0`.

**Question 3 — P0 special case**
Was this work produced during a P0 session, even if it is a directive aimed at a satellite?
If yes: `FROM: P0`. Always.
P0 never writes a satellite's name into its own FROM field on outputs it produced.

**Automation constraint (applies to I-48, I-31, D-06, and any future automated handoff pipeline):**
No `[FROM:]` label may be generated by an automated process without a human-readable origin justification logged alongside it. Format:

```
[FROM: P{#} — {project name}] [DATE: {YYYY-MM-DD}] [TOPIC: {brief}]
[ORIGIN: Work produced in {project name} session on {date} by {human or automation identifier}]
```

The `[ORIGIN:]` line is stripped before delivery to the receiving project but must be retained in the automation log. This provides an audit trail when handoff errors need to be traced.

**The test:** If you find yourself writing `FROM: P4` but the work was done in a P0 advisory session, stop. The correct label is `FROM: P0`. The fact that the content is about P4 topics does not change the FROM.
